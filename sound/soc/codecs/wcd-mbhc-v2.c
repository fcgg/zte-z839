...
{
    /**ZTE_MODIFY LiuTao 2016-02-19, fake button press while inserting headset */    
    ...
    pr_err(" %s: status(0x%x) mask(0x%x)\n", __func__, status, mask);
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
        ...
        if(jack_type & SND_JACK_HEADSET) {
    		...
    		dev_info(mbhc->codec->dev, "%s: liut delay btn det %d\n", __func__, mbhc->reject_btn);
    		cancel_delayed_work(&mbhc->btn_work);
					 ...
					 enum wcd_mbhc_plug_type plug_type)
{
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		wcd_mbhc_report_plug(mbhc, 1, SND_JACK_HEADSET); // SND_JACK_UNSUPPORTED
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (hs_comp_res && !(hphl_sch || mic_sch)) {
			...
			plug_type = MBHC_PLUG_TYPE_HIGH_HPH;
			wrk_complete = true;
            /**ZTE_MODIFY LiuTao 2016-03-04, for detect special headset */
		      ...
		      bool impedance_det_en)
{
	...
	int gnd_swh = 0;
	struct snd_soc_card *card = codec->card;
    struct proc_dir_entry *proc_hs_type;//weizhijun add headset detect, LiuTao 20150723, for pv & fastmmi
