				 ...
				 unsigned long event, void *data)
{
	...
	pr_info("%s enter, event=%lu, fb_sts=%d\n", __func__, event, fb_sts);
	return 0;
}
...
static void hall_timer_func (unsigned long _pwrkey)
{
	...
	if(open)
	{
		if(!hall_current_factory_mode && (fb_sts == FB_STATE_OFF))//not in factory mode and lcd is OFF
		{
	...
	{
		if(!hall_current_factory_mode && (fb_sts == FB_STATE_ON))// not in factory mode and lcd is ON
		{
			input_report_key(hallpwrkey->hall_pwr, KEY_POWER, 1);
			input_sync(hallpwrkey->hall_pwr);
			input_report_key(hallpwrkey->hall_pwr, KEY_POWER, 0);
