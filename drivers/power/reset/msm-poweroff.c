...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (qpnp_pon_check_hard_reset_stored()) {
		 ...
		 */
	pr_notice("[ZTE_BOOT]:use pon register save restart reason\n");
		if (get_dload_mode() ||
			((cmd != NULL && cmd[0] != '\0') &&
			strcmp(cmd, "recovery") &&
			...
			strcmp(cmd, "rtc") &&
			strcmp(cmd, "ftmmode") &&
			strcmp(cmd, "disemmcwp") &&
			strcmp(cmd, "emmcwpenab")))
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
    pr_info("enter msm_restart_probe()++++++++");
	...
	set_dload_mode(download_mode);
    pr_info("exit msm_restart_probe()----------");
