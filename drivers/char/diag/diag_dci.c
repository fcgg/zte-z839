	...
	for (i = 0; i < diag_max_reg && !found; i++) {
		...
		if (entry.cmd_code == header->cmd_code &&
			...
			if (entry.cmd_code_lo <= header->cmd_code &&
				 ...
				 */
#if 1 // zte ffbm demand, qcom_system_daemon should not deal, use modem side instead. And this will update with qualcomm later
				if (entry.cmd_code_lo == MODE_CMD &&
				    entry.cmd_code_hi == MODE_CMD &&
				    header->subsys_id == RESET_ID) {
					if (entry.client_id != APPS_DATA)	// only allow the app dealing with
						continue;
				}
				else if (entry.cmd_code_lo == MODE_CMD &&
				    entry.cmd_code_hi == MODE_CMD) {	// other non reset mode control
					if (entry.client_id == APPS_DATA)  // avoid app dealing with
						continue;
				}
#else
				if (entry.cmd_code_lo == MODE_CMD &&
