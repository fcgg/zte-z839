...
/*end-7 -diag security implementation-zhenghuan 2014/12/23*/
	...
	if (debug_diag_pkt) {
		pr_info("diag: In %s, received cmd %02x %02x %02x\n",
			__func__, cmd_code, subsys_id, subsys_cmd_code);
	}
			...
			if (entry.cmd_code == cmd_code && entry.subsys_id ==
				...
				if (entry.subsys_id ==
				   ...
				   entry.cmd_code_lo <=
					...
					if (debug_diag_pkt) {
						pr_info("diag: In %s, send_data %02x %02x %02x, client_id:%d\n",
							__func__, entry.cmd_code, entry.subsys_id, entry.cmd_code_hi, entry.client_id);
					}
