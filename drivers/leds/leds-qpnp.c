...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->mpp_cfg->pwm_mode == PWM_MODE) {
			...
			{
			#if (defined CONFIG_BOARD_MAX) || (defined CONFIG_BOARD_XRAY45) || (defined CONFIG_BOARD_XRAY50)//n9520A/N9131/N9133
                rc = pwm_config_us(led->mpp_cfg->pwm_cfg->pwm_dev,0.5*1000000,7.5*1000000);//blink 0.5s per 7.5s .SPRINT
            #else
