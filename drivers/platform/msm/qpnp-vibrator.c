...
struct qpnp_vib {
	...
	int vtg_level;
	int timeout;
	int timeout_ztelog;//zte_vib 20150326
	struct mutex lock;
};
       ...
       if(vibrator_voltage < QPNP_VIB_MAX_LEVEL&&  vibrator_voltage > QPNP_VIB_MIN_LEVEL)
		{
			...
			//pr_info("vib_voltage reset by user to %d\n",vibrator_voltage);
		}
...
static void qpnp_vib_enable(struct timed_output_dev *dev, int value)
{
	...
	if (value == 0)
		...
		value = (value > vib->timeout ?
				 vib->timeout : value);
		vib->timeout_ztelog= value; //zte_vib 20150326  add for log
		vib->state = 1;
